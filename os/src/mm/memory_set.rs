...
impl MemorySet {
    ...
    pub fn read_id(&self, id: usize) -> isize {
        let vpn: VirtPageNum = VirtAddr::from(id).floor();
        let pte = self.page_table.translate(vpn);
        if pte.is_none() || !pte.unwrap().is_valid() || !pte.unwrap().userable() || !pte.unwrap().readable(){
            return -1;
        } else {
            ...
            if let Some(pte) = self.translate(vpn) {
                ...
                unsafe { return (pa as *const u8).read() as isize; }
            }
        }
        -1
    }
    ...
    /// write_id
    pub fn write_id(&self, id: usize, _data: usize) -> isize {
        let vpn: VirtPageNum = VirtAddr::from(id).floor();
        let pte = self.page_table.translate(vpn);
        if pte.is_none() || !pte.unwrap().is_valid() || !pte.unwrap().userable() || !pte.unwrap().writable() {
            return -1;
        } else {
            let va = VirtAddr::from(id);
            let vpn = va.floor();
